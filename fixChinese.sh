...
if [ -e "/usr/share/applications/onlyoffice-desktopeditors.desktop" ]; then
  sed -ie 's/\/usr\/bin\/onlyoffice-desktopeditors/env QT_QPA_PLATFORM=xcb \/usr\/bin\/onlyoffice-desktopeditors --custom-title-bar/g' /usr/share/applications/onlyoffice-desktopeditors.desktop
fi
