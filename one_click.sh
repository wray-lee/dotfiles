...
#换源
read -p "是否换源为ali源,y or n" _source_
if [$_source_ == y];	then
rm /etc/yum.repos.d/*
...
#安装zsh和oh-my-zsh
read -p "是否安装zsh和oh-my-zsh,y or n"	_zsh_
if [$_zsh_ == y];	then
...
#git 配置邮箱用户名
echo "配置git"
read -p "输入您的名称和邮箱,用空格隔开" _name_	_email_
...
#安装docker
read -p "是否安装docker,y or n"	_docker_
if [$_docker_== y];	then
...
#安装nodejs
read -p "是否安装nodejs,y or n"	_nodejs_
if [$_nodejs_ == y];	then
